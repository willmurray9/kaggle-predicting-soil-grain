    ...
    for alpha in RIDGE_ALPHAS:
        ...
        for i in range(len(features)):
            ...
            predictions.append(ridge_curves(
                features[keep], curves[keep], features[i:i + 1], alpha=alpha,
            )[0])
...
def evaluate_nested_ridge(
    ...
    for i, sample_id in enumerate(sample_ids):
        ...
        selected, scores = select_ridge_alpha(features[keep], curves[keep])
        ...
        predictions = ridge_curves(features[keep], curves[keep], queries, alpha=selected)
